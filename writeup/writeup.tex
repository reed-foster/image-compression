...
In order to do this, the original image is transformed into a frequency representation.
...
\begin{IEEEeqnarray}{rCl}
    \mathbf{\widetilde{x}} & = & \mathbf{\Omega}\mathbf{x} \\
    \mathbf{\Omega} & = & \begin{pmatrix}\horzbar~\omega^T_1~\horzbar\\\vdots\\\horzbar~\omega^T_N~\horzbar\end{pmatrix}
\end{IEEEeqnarray}

Where $\omega_k$ are the basis vectors of the transform.
For a discrete fourier transform, these are simply complex exponentials, with different frequencies for different basis vectors:

\begin{equation}
    \omega_k = \begin{pmatrix} 1, e^{-\frac{2i\pi k}{N}}, e^{-\frac{4i\pi k}{N}}, \dots e^{\frac{2(N-1)\pi k}{N}} \end{pmatrix}^T
\end{equation}

Intuitively, a discrete transform of 2D data (e.g. a matrix) is performed by first taking a discrete transform of each individual column:
\begin{equation}
    \mathbf{\widetilde{X}}_c = \begin{pmatrix}
        \horzbar~\omega^T_1~\horzbar\\\vdots\\\horzbar~\omega^T_N~\horzbar
    \end{pmatrix}
    \begin{pmatrix}
        \vertbar && \vertbar \\
...
\begin{IEEEeqnarray}{rCl}
    \mathbf{\widetilde{X}}^T & = & \mathbf{\Omega}\mathbf{\widetilde{X}}^T_c \\
    \mathbf{\widetilde{X}} & = & \mathbf{\Omega}\mathbf{X}_c\mathbf{\Omega}^T
\end{IEEEeqnarray}

\subsection{Discrete Cosine Transform}

For a discrete cosine transform, the basis vectors are given by

\[
    \omega_k = \begin{pmatrix} \cos\frac{\pi k}{2N}, \cos\frac{3\pi k}{2N}, \dots \cos\frac{(2N-1)\pi k}{2N} \end{pmatrix}^T
\]
